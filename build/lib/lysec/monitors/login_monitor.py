...
class LoginMonitor(BaseMonitor):
    ...
    def __init__(self, config: dict, alert_engine):
        ...
        self._startup_backfill_enabled = bool(self._mon_cfg.get("startup_backfill_enabled", True))
        self._startup_backfill_lines = int(self._mon_cfg.get("startup_backfill_lines", 300))
        self._startup_backfill_records = int(self._mon_cfg.get("startup_backfill_records", 128))
    ...
    def setup(self):
        # Optional startup backfill to capture early boot/login activity.
        for path in self._watch_files:
            if os.path.isfile(path):
                if path.endswith(("wtmp", "btmp")):
                    if self._startup_backfill_enabled:
                        self._backfill_binary_log(path)
                    self._file_offsets[path] = os.path.getsize(path)
                else:
                    if self._startup_backfill_enabled:
                        self._backfill_text_log(path)
                    self._file_offsets[path] = os.path.getsize(path)
        logger.info(
    ...
    def _backfill_text_log(self, path: str):
        try:
            with open(path, "r", encoding="utf-8", errors="replace") as fh:
                lines = fh.readlines()
            if self._startup_backfill_lines > 0:
                lines = lines[-self._startup_backfill_lines :]
            for line in lines:
                self._parse_auth_line(line.strip())
        except Exception as exc:
            logger.debug("Startup text backfill failed for %s: %s", path, exc)

    def _backfill_binary_log(self, path: str):
        try:
            recs = max(1, int(self._startup_backfill_records))
            backfill_bytes = recs * UTMP_SIZE
            ...
            logger.debug("Startup binary backfill failed for %s: %s", path, exc)
