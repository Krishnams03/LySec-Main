...
class ProcessMonitor(BaseMonitor):
    ...
    def __init__(self, config: dict, alert_engine):
        ...
        self._alert_on_login_service_restarts = bool(
            self._mon_cfg.get("alert_on_login_service_restarts", True)
        )
        self._login_service_restart_window_sec = float(
            self._mon_cfg.get("login_service_restart_window_sec", 120.0)
        )
        self._login_service_restart_threshold = int(
            self._mon_cfg.get("login_service_restart_threshold", 3)
        )
        self._login_service_names = {
            str(x).lower()
            for x in self._mon_cfg.get(
                "login_service_names",
                ["gdm", "gdm3", "lightdm", "sddm", "xdm", "ly", "greetd", "getty", "agetty", "login"],
            )
        }
        ...
        self._login_service_history: dict[str, deque[float]] = defaultdict(deque)
    ...
    def _on_new_process(self, info: dict):
        ...
        self._check_login_service_restarts(name, exe, cmdline, base_details)
    ...
    def _check_login_service_restarts(
        self,
        name: str,
        exe: str,
        cmdline: str,
        ...
        if not self._alert_on_login_service_restarts:
            return

        candidates = {
            str(name or "").strip().lower(),
            os.path.basename(str(exe or "")).strip().lower(),
            os.path.basename(str(cmdline or "").split(" ", 1)[0]).strip().lower(),
        }
        matched = next((c for c in candidates if c and c in self._login_service_names), "")
        if not matched:
            return

        now = time.time()
        dq = self._login_service_history[matched]
        dq.append(now)
        self._prune_history(dq, now - self._login_service_restart_window_sec)
        if len(dq) >= self._login_service_restart_threshold:
            self._alert.fire(
                monitor="process",
                event_type="LOGIN_SERVICE_RESTART_BURST",
                message=(
                    f"Login service {matched} restarted {len(dq)} times in "
                    f"{self._login_service_restart_window_sec:.1f}s"
                ),
                severity=SEVERITY_HIGH,
                details={
                    **base_details,
                    "service_name": matched,
                    "restart_count": len(dq),
                    "restart_window_sec": self._login_service_restart_window_sec,
                },
            )
            dq.clear()
